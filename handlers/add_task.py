...
async def add_task_category(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            "Выберите категорию:", reply_markup=keyboards.category_keyboard()
        )
    except ValueError:
        await message.answer(
            "Неверный формат даты. Пожалуйста, укажите в формате ДД.ММ.ГГГГ ЧЧ:ММ"
        )
...
async def add_task_priority(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Выберите приоритет:", reply_markup=keyboards.priority_keyboard()
    )
...
async def save_task(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        "Задача успешно добавлена! ✅", reply_markup=ReplyKeyboardRemove()
    )
